"""Cattle Farmstead (cattle_farmstead): byre, hay barrack and a steaming manure heap round a farmyard.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/cattle_farmstead/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/cattle_farmstead

Identity (tier 1 of the cattle chain): stall feeding and manure yards carry the herd through the lean
months. A longer byre on a stone footing with a stable door, a hay barrack (pyramid thatch roof on
four tall poles over a stack of winter fodder), a steaming manure heap with a fork in it, a dun cow
in the trodden yard pulling hay from the barrack.

Local helpers (shared by the four cattle tiers): ``cow`` (side-view cattle in several coats,
grazing or standing, cow or bull), ``pasture``, ``yard``, ``post``, ``rail_fence``, ``hurdle``,
``muck`` (manure heap), ``hair``; ``thatch``/``daub`` from wheat_farm, ``planks``/``stones``/
``shingles``/``hay`` from tavern, ``tube``/``smooth``/``contour`` from elephant_kraal.
"""
...
def pole(ic: Icon, x: float, top: float, base: float, w: float = 26) -> None:
    rnd = ic.random
    pts = [(x - w / 2, base), (x - w / 2 + 3, top), (x + w / 2 - 3, top), (x + w / 2, base)]
    ic.fill(ic.poly_mask(pts), (132, 98, 64), (62, 42, 26), (x - w / 2, x + w / 2), vertical=False, noise=0.26)
    for y in np.arange(top + 40, base - 60, 46):  # peg holes for the roof pins
        ic.draw.ellipse((x - 4, y - 4 + rnd.uniform(-3, 3), x + 4, y + 4), fill=(34, 22, 12, 255))
    ic.outline(pts, 4)
...
def draw(ic: Icon) -> None:
    ...
    # ---- hay barrack: back poles, stack, front poles, pyramid thatch roof
    for x in (700, 902):
        pole(ic, x, 320, 720, 22)
    ic.shade(ic.mask("rectangle", (620, 330, 990, 470)), alpha=0.35, blur=18)
    stack = [(630, 772), (626, 500), (648, 456), (720, 432), (800, 424), (884, 432), (952, 456), (976, 500), (980, 772)]
    hay(ic, stack, strands=520)
    tint(ic, ic.poly_mask([(870, 420), (990, 420), (990, 780), (900, 780)]), ic.poly_mask(stack), (20, 10, 2), 0.3, 30)
    for x in (640, 966):
        pole(ic, x, 300, 790, 28)
    roof_eave = [(x, 346 + rnd.uniform(-5, 7)) for x in np.linspace(590, 1010, 16)]
    thatch(ic, [(786, 150), (800, 146), (814, 150)], roof_eave, c=("#9c8a64", "#4a3e2a"))
    ic.shade(ic.poly_mask([(800, 140), (1020, 360), (820, 380)]), alpha=0.22, blur=30)
    ic.line([(800, 150), (800, 104)], 12, (98, 72, 46))
    ic.shade(ic.mask("rectangle", (620, 372, 990, 420)), alpha=0.45, blur=12)

    # ---- byre: long, on a stone footing, stable door, loft hatch under the thatch
    ic.shade(ic.mask("ellipse", (20, 680, 640, 770)), alpha=0.4, blur=16, clip=False)
    daub(ic, (70, 470, 574, 690), posts=(76, 230, 400, 568))
    stones(ic, (60, 680, 584, 736), "#86786a", "#524638", course=28, moss=0.14)
    ic.outline([(60, 680), (584, 680), (584, 736), (60, 736)], 4)
    # stable door: upper leaf open (dark), lower leaf shut
    dx0, dx1 = 262, 368
    ic.rect((dx0 - 14, 528, dx1 + 14, 736), "#7a5436", "#4a3220", edge=4)
    ic.rect((dx0, 540, dx1, 736), "#201610", "#0a0604", noise=0.1, edge=0)
    lower = ic.mask("rectangle", (dx0, 630, dx1, 736))
    planks(ic, (dx0, 630, dx1, 736), "#94704c", "#56402a", board=24, clip=lower)
    ic.beam((dx0 + 6, 640), (dx1 - 6, 640), 10, (84, 58, 36))
    ic.beam((dx0 + 6, 650), (dx1 - 6, 726), 10, (84, 58, 36))
    ic.outline([(dx0, 630), (dx1, 630), (dx1, 736), (dx0, 736)], 4)
    ic.window(450, 536, 500, 578)
    ic.window(130, 536, 176, 578)
    ic.shade(ic.mask("rectangle", (70, 470, 574, 530)), alpha=0.5, blur=12)
    ridge = [(170, 250), (320, 242), (470, 252)]
    eave = [(x, 486 + rnd.uniform(-6, 8)) for x in np.linspace(24, 624, 22)]
    thatch(ic, ridge, eave, c=("#a88c58", "#5a4626"))
    ic.shade(ic.poly_mask([(400, 230), (640, 230), (640, 500), (440, 500)]), alpha=0.2, blur=40)

    # ---- farmyard (straw thinned in the front half; no sky puddles)
    top = [(8, 760)] + [(x, 740 + rnd.uniform(-8, 8)) for x in np.linspace(80, 950, 14)] + [(1016, 760)]
    ground = yard(ic, top, 948, colors=("#6e5238", "#3a2818"), front_straw=0.5)
    ic.shade(ic.mask("ellipse", (20, 720, 640, 790)), alpha=0.3, blur=16)
    # small dark muddy puddle by her hooves: churned mud, a dull wet sheen, no sky
    pm = ic.poly_mask(ic.jitter(588, 934, 40, 11, 12, 0.14))
    tint(ic, ic.poly_mask(ic.jitter(588, 934, 60, 18, 12, 0.18)), ground, (16, 10, 4), 0.4, 6)
    ic.fill(pm, "#3a2a1a", "#1a120a", (922, 946), noise=0.14)
    tint(ic, ic.poly_mask(ic.jitter(582, 928, 26, 4, 8, 0.1)), pm, (132, 108, 76), 0.3, 2)

    # ---- manure heap, front left: a domed mound with a lit crown, steam rising against the byre wall,
    # a fork pushed into its top at an angle
    # damp dark stain on the byre wall behind the heap, so its steam shows against it
    ic.shade(ic.mask("ellipse", (-40, 500, 400, 800)), (34, 26, 16), 0.4, blur=34)
    heap = muck(ic, 26, 334, 924, 704, wisps=((-0.24, 190, 0.55), (0.0, 250, 0.55), (0.22, 200, 0.5)))
    ex, ey = 204, 716  # where the tines enter the heap
    hx, hy = 116, 548  # end of the handle
    L = math.hypot(ex - hx, ey - hy)
    ux, uy = (ex - hx) / L, (ey - hy) / L
    ic.line([(hx, hy), (ex, ey)], 13, (40, 28, 20))
    ic.line([(hx, hy), (ex, ey)], 8, (150, 112, 72))
    ic.line([(hx + 2, hy + 1), (ex - 12 * ux - 3, ey - 12 * uy)], 3, (230, 200, 150, 120))
    cb = [(ex - uy * 22, ey + ux * 22), (ex + uy * 22, ey - ux * 22)]
    ic.line(cb, 9, (58, 56, 54))
    for k in (-1, 0, 1):
        bx, by = ex + (-uy) * 18 * k, ey + ux * 18 * k
        ic.line([(bx, by), (bx + ux * 24, by + uy * 24)], 6, (70, 68, 64))
    # bedding pushed up over the buried tines
    lump = ic.poly_mask(ic.jitter(ex + ux * 28, ey + uy * 26, 40, 16, 10, 0.2))
    lump = ic.intersect(lump, heap.filter(ImageFilter.MaxFilter(9)))
    ic.fill(lump, "#7a5a36", "#3a2814", (ey, ey + 40), noise=0.35)

    def bedding(d):
        for _ in range(26):
            x, y = rnd.uniform(ex - 20, ex + 54), rnd.uniform(ey + 6, ey + 42)
            a = rnd.uniform(0, math.pi)
            d.line([(x, y), (x + math.cos(a) * 16, y + math.sin(a) * 6)],
                   fill=rnd.choice([(214, 180, 104, 210), (170, 134, 70, 190)]), width=3)

    ic.overlay(bedding, lump)

    # ---- the dun cow, turned to the hay barrack, pulling hay from the stack (its foot in shade behind her)
    tint(ic, ic.mask("rectangle", (600, 560, 1000, 800)), ic.poly_mask(stack), (30, 18, 6), 0.35, 30)
    cow(ic, 722, 918, 0.92, left=False, coat="dun")
    mx, my = 722 + 173 * 0.92, 918 - 175 * 0.92

    def wisp(d):
        for _ in range(16):
            x, y = mx + rnd.uniform(-10, 14), my + rnd.uniform(-4, 14)
            a = rnd.uniform(0.6, 2.4)
            ln = rnd.uniform(18, 38)
            d.line([(x, y), (x + math.cos(a) * ln * 0.6, y + math.sin(a) * ln)],
                   fill=rnd.choice([(214, 184, 110, 255), (170, 138, 72, 255), (236, 210, 140, 255)]), width=3)
