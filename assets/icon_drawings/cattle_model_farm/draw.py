"""Cattle Model Farm (cattle_model_farm): a great slate-roofed stone barn with a dairy wing and a bull.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/cattle_model_farm/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/cattle_model_farm

Identity (tier 3 of the cattle chain): selected breeding stock, dairies and fodder leys raise cattle
on a larger managed farm. A long stone barn under a blue slate roof with a gabled wagon porch and a
louvred ventilator on the ridge, a whitewashed dairy wing, a neat white-railed paddock in front
with a pied dairy cow and a ringed red bull, milk churns at the paddock gate.

Local helpers (shared by the four cattle tiers): ``cow`` (side-view cattle in several coats,
grazing or standing, cow or bull), ``pasture``, ``yard``, ``crown``, ``post``, ``rail_fence``,
``hurdle``, ``muck`` (manure heap), ``hair``; ``thatch``/``daub`` from wheat_farm, ``planks``/
``stones``/``shingles``/``hay`` from tavern, ``tube``/``smooth``/``contour`` from elephant_kraal.
New here: ``churn``.
"""
...
def muck(ic: Icon, x0: float, x1: float, base: float, top: float, steam: bool = True,
    ...
    if steam:
        for k, (dx, h, alpha) in enumerate(wisps):
            ...
            ic.shade(wisp, (236, 230, 220), alpha, clip=False)
    return m
...
SEED = 43
REFS = ("sheep_farms", "farming_village", "fulani_cattle_rearing_pen", "horse_breeders")

SLATE = ("#6e7e90", "#2e3846")
BASE = 772


def churn(ic: Icon, cx: float, base: float, w: float, h: float) -> None:
    """Tinned milk churn: cylinder with a sloping shoulder, neck, lid and handles; metal lit left."""
    x0, x1 = cx - w / 2, cx + w / 2
    sh = base - h * 0.62
    body = [(x0, base), (x0, sh), (cx - w * 0.28, base - h * 0.84), (cx - w * 0.28, base - h * 0.92),
            (cx + w * 0.28, base - h * 0.92), (cx + w * 0.28, base - h * 0.84), (x1, sh), (x1, base)]
    tint(ic, ic.mask("ellipse", (x0 - 6, base - 12, x1 + 20, base + 12)), None, (0, 0, 0), 0.45, 6)
    m = ic.poly_mask(body)
    ic.fill(m, "#dcdcd4", "#50545a", (x0 + w * 0.1, x1), vertical=False, noise=0.08, chroma=0.02)
    tint(ic, ic.mask("rectangle", (x0 + w * 0.14, base - h, x0 + w * 0.26, base)), m, (255, 255, 250), 0.35, 4)
    for f in (0.1, 0.5):
        y = base - h * f
        ic.line([(x0, y), (x1, y)], 7, (90, 92, 96))
        ic.line([(x0, y - 3), (cx, y - 3)], 3, (240, 240, 236, 150))
    lid = (cx - w * 0.34, base - h * 1.0, cx + w * 0.34, base - h * 0.9)
    ic.fill(ic.mask("rectangle", lid), "#c8c8c2", "#5a5e62", noise=0.08)
    ic.outline([(lid[0], lid[1]), (lid[2], lid[1]), (lid[2], lid[3]), (lid[0], lid[3])], 3)
    ic.outline(body, 4)
    for sx in (-1, 1):
        ic.overlay(lambda d, sx=sx: d.arc((cx + sx * w * 0.5 - 12, sh - 10, cx + sx * w * 0.5 + 12, sh + 14),
                                          0, 360, fill=(60, 62, 66, 255), width=5))
...
def draw(ic: Icon) -> None:
    ...
    # ---- trees behind the steading
    crown(ic, 90, 420, 110)
    crown(ic, 960, 420, 100)

    # ---- great barn: stone walls, slate roof, louvred ventilator on the ridge
    X0, X1, EV, RG = 110, 720, 452, 214
    ic.shade(ic.mask("ellipse", (80, BASE - 40, 1000, BASE + 40)), alpha=0.45, blur=16, clip=False)
    wall = ic.mask("rectangle", (X0, EV, X1, BASE))
    stones(ic, (X0, EV, X1, BASE), "#c6aa80", "#86684a", course=34, moss=0.05)
    for x in (180, 246, 596, 660):  # ventilation slits
        ic.rect((x - 9, 540, x + 9, 640), "#241a14", "#0c0806", noise=0.08, edge=3)
        tint(ic, ic.mask("rectangle", (x - 14, 640, x + 14, 656)), wall, (0, 0, 0), 0.3, 4)
    tint(ic, ic.mask("rectangle", (X0, BASE - 60, X1, BASE)), wall, (40, 44, 20), 0.22, 16)
    tint(ic, ic.mask("rectangle", (X0, EV, X1, EV + 70)), wall, (0, 0, 0), 0.5, 14)
    ic.outline([(X0, EV), (X1, EV), (X1, BASE), (X0, BASE)], 5)
    # ventilator behind the roof plane
    vx = 414
    ic.rect((vx - 44, RG - 70, vx + 44, RG + 10), "#3a2c20", "#1a120c", edge=4)
    for y in np.arange(RG - 60, RG + 4, 14):
        ic.line([(vx - 40, y), (vx + 40, y + 6)], 8, (150, 140, 124))
    shingles(ic, [(vx - 64, RG - 64), (vx + 64, RG - 64), (vx, RG - 128)], SLATE[0], SLATE[1], course=18,
             stagger=22, edge=5)
    ic.line([(vx, RG - 128), (vx, RG - 176)], 5, (40, 36, 34))
    ic.poly([(vx, RG - 170), (vx + 34, RG - 162), (vx, RG - 154)], "#4a4440", "#2a2624", edge=3)
    roof = [(X0 - 40, EV + 14), (X1 + 40, EV + 14), (X1 - 60, RG), (X0 + 60, RG)]
    rm = shingles(ic, roof, SLATE[0], SLATE[1], course=26, stagger=34, edge=7)
    tint(ic, ic.poly_mask([(560, RG), (X1 + 60, RG), (X1 + 60, EV + 20), (600, EV + 20)]), rm, (0, 0, 0), 0.2, 40)
    tint(ic, ic.poly_mask([(X0, RG), (300, RG), (240, EV), (X0 - 40, EV)]), rm, (240, 244, 250), 0.1, 40)
    ic.rect((X0 + 56, RG - 12, X1 - 56, RG + 8), "#6a5a50", "#3e342e", edge=4)
    # wagon porch: gable to the front, great doors open on the dark threshing floor
    P0, P1, PA, PE = 318, 510, 318, 478
    pw = ic.mask("rectangle", (P0, PE, P1, BASE))
    stones(ic, (P0, PE, P1, BASE), "#ceb48a", "#8c7050", course=30, moss=0.03)
    ic.outline([(P0, PE), (P1, PE), (P1, BASE), (P0, BASE)], 5)
    dm = union(ic.mask("rectangle", (P0 + 30, 590, P1 - 30, BASE)), ic.mask("ellipse", (P0 + 30, 530, P1 - 30, 650)))
    ic.fill(dm, "#241a12", "#0a0604", noise=0.08)
    tint(ic, ic.mask("rectangle", (P0, 520, P1, 620)), dm, (0, 0, 0), 0.4, 12)
    ic.overlay(lambda d: d.arc((P0 + 30, 530, P1 - 30, 650), 180, 360, fill=(230, 220, 200, 200), width=10))
    for side, x0, x1 in ((-1, P0 + 4, P0 + 30), (1, P1 - 30, P1 - 4)):  # door leaves folded back
        planks(ic, (x0, 600, x1, BASE), "#8a6a48", "#4a3622", board=13)
    gable = [(P0 - 34, PE + 16), (P1 + 34, PE + 16), ((P0 + P1) / 2, PA)]
    shingles(ic, [(P0 - 34, PE + 16), ((P0 + P1) / 2, PA), ((P0 + P1) / 2, PA + 36), (P0 - 10, PE + 20)],
             SLATE[0], SLATE[1], course=18, stagger=24, edge=5)
    pr = shingles(ic, [(P1 + 34, PE + 16), ((P0 + P1) / 2, PA), ((P0 + P1) / 2, PA + 36), (P1 + 10, PE + 20)],
                  SLATE[0], SLATE[1], course=18, stagger=24, edge=5)
    tint(ic, pr, pr, (0, 0, 0), 0.2)
    gm = ic.poly_mask([(P0 - 6, PE + 12), (P1 + 6, PE + 12), ((P0 + P1) / 2, PA + 40)])
    planks(ic, (P0, PA, P1, PE + 16), "#9c7c58", "#5a4430", board=20, clip=gm)
    ic.rect(((P0 + P1) / 2 - 20, PE - 70, (P0 + P1) / 2 + 20, PE - 34), "#2a1c12", "#0e0806", edge=3)
    tint(ic, ic.mask("rectangle", (P0, PE - 90, P1, PE + 20)), gm, (0, 0, 0), 0.3, 10)
    tint(ic, ic.mask("rectangle", (P0, PE + 16, P1, PE + 60)), pw, (0, 0, 0), 0.45, 10)
    tint(ic, ic.mask("rectangle", (P1, PE, P1 + 50, BASE)), wall, (0, 0, 0), 0.35, 12)

    # ---- dairy wing: whitewashed, lower slate roof, louvred window
    D0, D1, DE, DR = 700, 1004, 596, 500
    dw = ic.mask("rectangle", (D0, DE, D1, BASE))
    ic.rect((D0, DE, D1, BASE), "#eadcbc", "#b49c78", noise=0.2, edge=0)
    for _ in range(12):
        x, y = rnd.uniform(D0, D1), rnd.uniform(DE, BASE)
        r = rnd.uniform(12, 30)
        tint(ic, ic.mask("ellipse", (x - r * 1.5, y - r, x + r * 1.5, y + r)), dw,
             (255, 250, 236) if rnd.random() < 0.5 else (90, 80, 60), 0.14, 6)
    tint(ic, ic.mask("rectangle", (D0, BASE - 50, D1, BASE)), dw, (60, 60, 30), 0.3, 12)
    tint(ic, ic.mask("rectangle", (D0, DE, D1, DE + 50)), dw, (0, 0, 0), 0.45, 12)
    tint(ic, ic.mask("rectangle", (D0, DE, D0 + 50, BASE)), dw, (0, 0, 0), 0.35, 14)
    lx0, lx1 = 770, 842
    ic.rect((lx0 - 10, 640, lx1 + 10, 712), "#6a5a48", "#3e3228", edge=4)
    ic.rect((lx0, 650, lx1, 702), "#241c16", "#0c0806", edge=0)
    for y in np.arange(654, 702, 11):
        ic.line([(lx0, y), (lx1, y + 5)], 6, (170, 160, 140))
    ic.door(904, 656, 956, BASE, arched=False, surround=None)
    ic.outline([(D0, DE), (D1, DE), (D1, BASE), (D0, BASE)], 5)
    shingles(ic, [(D0 - 10, DE + 12), (D1 + 18, DE + 12), (D1 + 4, DR), (D0 + 40, DR)], SLATE[0], SLATE[1],
             course=22, stagger=28, edge=6)
    ic.rect((940, DR - 60, 976, DR + 10), "#b0a490", "#76695a", edge=4)  # dairy flue
    ic.rect((934, DR - 70, 982, DR - 56), "#9a8e7c", "#5e5448", edge=3)

    # ---- paddock
    top = [(10, 796)] + [(x, 780 + rnd.uniform(-6, 6)) for x in np.linspace(80, 950, 14)] + [(1014, 796)]
    pasture(ic, top, 960, colors=("#7e8a42", "#465020"), tufts=20)
    ic.shade(ic.mask("rectangle", (60, 770, 1010, 820)), alpha=0.35, blur=14)

    # ---- breeding stock: red bull and pied dairy cow
    cow(ic, 290, 910, 0.68, left=False, coat="chestnut", bull=True)
    cow(ic, 640, 918, 0.7, left=True, coat="pied")

    # ---- white rails and churns at the gate
    posts = [(24, 944), (270, 948), (520, 946), (770, 948)]
    rail_fence(ic, posts, 128, rails=(0.32, 0.74), w=30, rail_w=18, wood=((228, 222, 206), (140, 134, 120)))
    churn(ic, 878, 950, 66, 130)
    churn(ic, 952, 942, 62, 120)
