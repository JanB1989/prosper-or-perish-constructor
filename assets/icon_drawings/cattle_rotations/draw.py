"""Cattle Ley Farm (cattle_rotations): fenced strips of sown grass and clover leys with cattle grazing.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/cattle_rotations/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/cattle_rotations

Identity (tier 2 of the cattle chain): sown leys and fodder crops let the herd graze fields that rest
from grain. Field strips running away from the viewer in different greens (grass ley, flowering
clover ley, a golden grain strip), post-and-rail fences between them, a red cow grazing and a pied
cow standing in the front ley, more cattle on the far ley, a tiled timber-framed barn at the head of
the fields.
...
FB, FF = 548, 890  # field back / front edge
...
def ley(ic: Icon, bl, br, fl, fr, colors, flowers=None, grain: bool = False) -> Image.Image:
    """A field strip in perspective: sown grass (strokes larger towards the front), optional clover
    flowers, or a golden grain strip with ear dashes."""
    rnd = ic.random
    m = ic.poly_mask([bl, br, fr, fl])
    ic.fill(m, colors[0], colors[1], (bl[1], fl[1]), noise=0.3, chroma=0.12)
    for _ in range(14):
        t = rnd.random()
        a, b = lerp(bl, fl, t), lerp(br, fr, t)
        x, y = lerp(a, b, rnd.random())
        r = rnd.uniform(20, 50) * (0.5 + t)
        tint(ic, ic.mask("ellipse", (x - r * 1.6, y - r * 0.4, x + r * 1.6, y + r * 0.4)), m,
             rnd.choice([(255, 240, 190), (16, 24, 6)]), rnd.uniform(0.1, 0.2), 8)
    ...
    def paint(d):
        ...
        while y < fl[1]:
            ...
            x = xa + rnd.uniform(0, 8)
            while x < xb:
                if grain:
                    col = rnd.choice([(222, 196, 124, 170), (130, 96, 40, 180), (196, 164, 88, 170)])
                    d.line([(x, y), (x + rnd.uniform(-2, 2), y - 16 * s)], fill=col, width=max(2, int(5 * s)))
                else:
                    col = rnd.choice([(190, 196, 110, 120), (40, 60, 18, 150), (130, 150, 60, 130)])
                    d.line([(x, y), (x + rnd.uniform(-5, 5), y - rnd.uniform(8, 18) * s)], fill=col, width=3)
                    if flowers and rnd.random() < 0.05:
                        r = 8 * s + 3
                        col = flowers[rnd.randrange(len(flowers))]
                        d.ellipse((x - r, y - r * 0.8 - 6 * s, x + r, y + r * 0.8 - 6 * s), fill=col + (235,))
                        d.ellipse((x - r * 0.5, y - r * 0.9 - 6 * s, x + r * 0.3, y - r * 0.2 - 6 * s),
                                  fill=(255, 250, 240, 110))
                x += rnd.uniform(8, 14) * s
            y += 9 * s
...
def fence_line(ic: Icon, p_back, p_front, n: int, h_back: float = 34, h_front: float = 110) -> None:
    """Post-and-rail fence running along a strip border into the distance."""
    pts = []
    for k in range(n):
        t = (k / (n - 1)) ** 1.25
        x, y = lerp(p_back, p_front, t)
        pts.append((x, y, h_back + (h_front - h_back) * t, 6 + 18 * t))
    for (xa, ya, ha, wa), (xb, yb, hb, wb) in zip(pts, pts[1:]):
        for fr in (0.4, 0.8):
            ic.line([(xa, ya - ha * fr), (xb, yb - hb * fr)], int(4 + wb * 0.5), (40, 28, 20, 220))
            ic.line([(xa, ya - ha * fr), (xb, yb - hb * fr)], int(2 + wb * 0.3), (150, 118, 82))
    for x, y, hh, w in pts:
        ic.rect((x - w / 2, y - hh, x + w / 2, y + 2), (140, 106, 72), (70, 48, 30), vertical=False, noise=0.2,
                edge=3)
...
def draw(ic: Icon) -> None:
    ...
    # ---- hedgerow trees behind the barn and fields
    for cx, cy, r in ((120, 460, 112), (880, 470, 122), (990, 512, 82)):
        crown(ic, cx, cy, r)

    # ---- barn at the head of the fields: timber frame, tiled roof, big cart door
    X0, X1, EV, RG = 250, 700, 398, 238
    ic.shade(ic.mask("ellipse", (220, 560, 740, 610)), alpha=0.4, blur=12, clip=False)
    ic.plaster_frame((X0, EV, X1, 580), posts=(X0 + 12, 380, 520, X1 - 12), rails=(492,),
                     braces=(((380, 580), (446, 492)),), wall="#d8c6a0", wall_dark="#a8926c")
    wall = ic.mask("rectangle", (X0, EV, X1, 580))
    ic.rect((440, 450, 580, 580), "#2a1c12", "#0e0806", noise=0.1, edge=0)
    planks(ic, (440, 450, 486, 580), "#8e6c48", "#503a24", board=22)
    ic.beam((432, 446), (588, 446), 16, (100, 70, 44))
    ic.window(300, 436, 340, 470)
    tint(ic, ic.mask("rectangle", (X0, EV, X1, EV + 50)), wall, (0, 0, 0), 0.45, 12)
    roof = [(X0 - 40, EV + 16), (X1 + 40, EV + 16), (X1 - 20, RG), (X0 + 20, RG)]
    shingles(ic, roof, "#a4644a", "#5a3226", course=24, stagger=32, edge=6)
    ic.rect((X0 + 16, RG - 12, X1 - 16, RG + 6), "#7a4a36", "#4e2e20", edge=4)
    ic.shade(ic.poly_mask([(560, RG - 20), (760, RG - 20), (760, EV + 30), (600, EV + 30)]), alpha=0.2, blur=40)

    # ---- the rotation: strips running away from the viewer, converging strongly towards the barn,
    # different in value as well as hue: dark grass ley, pale clover ley, golden grain strip
    field = ic.poly_mask([(110, FB - 6), (930, FB - 6), (1008, FF), (16, FF)])
    ley(ic, (110, FB - 6), (930, FB - 6), (1008, FF), (16, FF), ("#6e7c38", "#3e4a1c"))  # headland meadow
    bx0, bx1, fx0, fx1 = 262, 790, 16, 1008
    kinds = [("L", ("#566a2a", "#27340e")), ("G", ("#c09a4c", "#735826")), ("C", ("#a4b464", "#63722e")),
             ("L", ("#6c7e34", "#36441a"))]
    widths = [0.28, 0.16, 0.26, 0.30]
    cum = np.concatenate([[0], np.cumsum(widths) / sum(widths)])
    strips = []
    for k, (kind, cols) in enumerate(kinds):
        bl, br = (bx0 + (bx1 - bx0) * cum[k], FB), (bx0 + (bx1 - bx0) * cum[k + 1], FB)
        fl, fr = (fx0 + (fx1 - fx0) * cum[k], FF), (fx0 + (fx1 - fx0) * cum[k + 1], FF)
        strips.append((bl, br, fl, fr))
        ley(ic, bl, br, fl, fr, cols, flowers=((246, 242, 228), (226, 150, 176)) if kind == "C" else None,
            grain=kind == "G")
    ic.shade(ic.intersect(field, ic.mask("rectangle", (0, FB, SIZE, FB + 80))), (230, 220, 180), 0.22, blur=24)
    ic.poly([(fx0, FF), (fx1, FF), (fx1 - 8, FF + 34), (fx0 + 8, FF + 34)], "#5e4428", "#3a2a18", noise=0.3, edge=3)

    # ---- small cattle on the far leys
    cow(ic, 850, 616, 0.2, left=False, coat="red", graze=True)
    cow(ic, 250, 598, 0.2, left=True, coat="red", legs=0.72)

    # ---- fences along the strip borders
    for k in (1, 3):
        fence_line(ic, (bx0 + (bx1 - bx0) * cum[k], FB), (fx0 + (fx1 - fx0) * cum[k], FF + 6), 6)

    # ---- cattle in the front leys, behind the front fence
    cow(ic, 332, 856, 0.68, left=False, coat="red", graze=True)
    cow(ic, 660, 878, 0.83, left=True, coat="pied")
    posts = [(30, FF + 26), (262, FF + 30), (500, FF + 28), (744, FF + 30), (994, FF + 26)]
    rail_fence(ic, posts, 132, rails=(0.32, 0.74), w=34, rail_w=22)
