"""Legume Farm (tier 0 of the legume farm chain): a low thatched cottage behind rows of bushy peas and beans.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/legume_farm/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/legume_farm

Identity: low glaucous-green pulse bushes hung with fat pale pods fill the foreground; an open
sack heaped with mixed beans and a basket of picked pods sit at the bottom right. The building is
the humblest of the chain: one wattle-and-daub cottage under a heavy, weathered thatch.

Local helpers shared by the legume family: ``pulse_bush`` (dome of leaflets, flowers, hanging
pods), ``pod``, ``beans`` / ``bean_heap_items`` (mixed pulses), ``bean_sack`` (open sack heaped
with beans), ``soil``; generic ones copied from cookshop / tavern / victualling_yard.
"""
...
SEED = 11
REFS = ("farming_village", "fiber_crops_farm", "free_village", "fruit_orchard")


def cottage(ic: Icon) -> None:
    rnd = ic.random
    L, R, EAVE, BASE = 170, 650, 500, 800
    # wattle-and-daub walls: ochre plaster, a patch of bare wattle, a few timber posts
    wall = ic.mask("rectangle", (L, EAVE - 20, R, BASE))
    ic.fill(wall, "#c8b084", "#8c7452", (EAVE, BASE), noise=0.22, chroma=0.06)
    for _ in range(10):
        x, y = rnd.uniform(L, R), rnd.uniform(EAVE, BASE)
        r = rnd.uniform(20, 50)
        tint(ic, ic.mask("ellipse", (x - r * 1.4, y - r * 0.7, x + r * 1.4, y + r * 0.7)), wall,
             rnd.choice([(80, 64, 40), (255, 244, 220), (110, 110, 70)]), rnd.uniform(0.08, 0.18), 14)
    patch = ic.poly_mask([(540, 600), (610, 592), (620, 680), (532, 690)])

    def wattle(d):
        for y in range(592, 694, 14):
            d.line([(525, y), (625, y + 3)], fill=(110, 80, 50, 255), width=9)
            d.line([(525, y - 3), (625, y)], fill=(170, 130, 86, 160), width=3)
        for x in (552, 580, 606):
            d.line([(x, 588), (x + 2, 694)], fill=(70, 50, 30, 255), width=7)

    ic.overlay(wattle, patch)
    ic.outline([(540, 600), (610, 592), (620, 680), (532, 690)], 3, (60, 44, 26, 150))
    tint(ic, ic.mask("rectangle", (L, BASE - 70, R, BASE)), wall, (40, 34, 20), 0.35, 20)  # damp, grime at the foot
    for x in (L + 12, 440, R - 12):
        timber(ic, (x, EAVE - 10), (x + rnd.uniform(-3, 3), BASE), 24)
    timber(ic, (L, BASE - 8), (R, BASE - 6), 20)
    ic.door(290, 610, 370, BASE - 16, arched=False, surround=None)
    ic.window(470, 596, 516, 640)
    # heavy thatch hanging low over the walls, hipped, slightly sagging ridge
    roof = [(L - 60, EAVE + 26), (R + 60, EAVE + 30), (R - 50, 270), (500, 254), (330, 260), (L + 60, 274)]
    thatch(ic, roof, "#c4953e", "#5a3c14", 40)
    ic.shade(ic.poly_mask([(R - 50, 270), (R + 60, EAVE + 30), (R - 40, EAVE + 30)]), (20, 12, 4), 0.32, blur=3)
    ic.shade(ic.poly_mask([(L + 60, 274), (L - 60, EAVE + 26), (L + 40, EAVE + 26)]), (255, 236, 196), 0.14, blur=3)
    ic.shade(ic.mask("rectangle", (L, EAVE + 20, R, EAVE + 80)), (20, 12, 4), 0.5, blur=12)  # eave shadow on the wall
    ic.shade(ic.mask("rectangle", (L, EAVE + 40, R, EAVE + 64)), (16, 8, 2), 0.72, blur=3)  # deep band under the overhang
    ic.shade(ic.poly_mask([(470, 300), (R + 80, 300), (R + 80, EAVE + 40), (520, EAVE + 40)]), alpha=0.18, blur=50)
    # the eave edge: a thick cut straw lip, lifted a little at both ends
    eave = [(x, EAVE + 24 + (x - L) * 0.01 - 14 * ((x - (L + R) / 2) / ((R - L) / 2 + 60)) ** 4 + rnd.uniform(-4, 5))
            for x in np.linspace(L - 60, R + 60, 22)]
    lip = eave + [(x, y + 24 + rnd.uniform(-3, 4)) for x, y in eave[::-1]]
    ic.fill(ic.poly_mask(lip), "#9a8250", "#4a3a1e", noise=0.35)
    ic.line([(x, y + 3) for x, y in eave], 5, (236, 206, 140, 110))  # lit upper edge of the lip
    ic.outline(roof, 6)
    ic.line([(L + 60, 274), (330, 260), (500, 254), (R - 50, 270)], 16, (112, 96, 60))
    # smoke hole cap on the ridge
    ic.poly([(430, 256), (470, 254), (476, 226), (426, 228)], "#8a7a58", "#4e422c", edge=4)
...
def draw(ic: Icon) -> None:
    ...
    # the field: soil strip with three rows of separate pulse bushes, smaller at the back
    ground = [(40, 770), (800, 764), (840, 1000), (22, 1000)]
    soil(ic, ground, furrows=[[(50, y), (820, y - 4)] for y in (806, 872, 950)])
    rows = ((806, (120, 250, 380, 510, 640, 760), 112, 76, 0),
            (878, (100, 270, 440, 610, 760), 150, 110, 3),
            (990, (150, 400, 640), 236, 196, 5))
    for y, xs, bw, bh, npods in rows:
        for x in xs:
            pulse_bush(ic, x + rnd.uniform(-10, 10), y + rnd.uniform(-4, 4), bw * rnd.uniform(0.92, 1.08),
                       bh * rnd.uniform(0.92, 1.08), pods=npods, flowers=1 if bh < 150 else 2)
    ic.shade(ic.mask("rectangle", (0, 975, 900, 1024)), (10, 18, 4), alpha=0.3, blur=16)

    # harvest, bottom right: basket of picked pods, open sack heaped with beans
    ic.shade(ic.mask("ellipse", (700, 950, 1016, 1002)), alpha=0.45, blur=10)
    bean_sack(ic, 900, 996, 190, 230, weights=(2, 3, 2, 3, 1), bean=13)
    ic.basket(724, 880, 866, 996)
    for k in range(6):
        x = 750 + k * 19 + rnd.uniform(-5, 5)
        ang = rnd.uniform(-20, 20) + (0 if k % 2 else 180)
        pod(ic, (x - 34 * math.cos(math.radians(ang)), 872 + rnd.uniform(-8, 4)), ang, 72, 21,
            POD if k != 3 else POD_DRY, 4, 0.14)
