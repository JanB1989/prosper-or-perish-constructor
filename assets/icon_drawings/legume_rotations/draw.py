"""Pulse Rotation Farm (tier 2 of the legume farm chain): a field laid out in rotation strips of beans,
grain, fresh furrows and flowering vetch, behind a hedgerow a stone farmhouse and a tiled barn.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/legume_rotations/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/legume_rotations

Identity: a slab of farmland split into wedge-shaped strips running away from the viewer, so the
rotation reads at a glance: podded pulse bushes on the left (the chain's crop), golden grain,
ploughed fallow and purple-flowered vetch. Hedgerow, stone farmhouse and plank barn with tiled roofs
behind show the larger improved holding; sacks of beans at the bottom right.

Local helpers shared by the legume family: ``pulse_bush``, ``pod``, ``beans`` / ``bean_heap_items``,
``bean_sack``, ``soil``; field slab helpers (``Field``, ``slab_face``, ``grain``, ``furrows``) from
field_management, ``hedge`` new here; generic ones copied from cookshop / tavern / victualling_yard.
"""
...
def grain(ic: Icon, F: Field, v0: float, v1: float, u0=0.0, u1=1.0, h: float = 54, colors=GOLD) -> Image.Image:
    """Standing cereal in straw gold: stalk strokes, darker ear heads, lighter at the back, darker in front."""
    rnd = ic.random
    m = F.mask(ic, v0, v1, u0, u1)
    ys = [p[1] for p in F.band(v0, v1, u0, u1)]
    ic.fill(m, colors[0], colors[1], (min(ys) - 20, max(ys) + 30), noise=0.26, chroma=0.06)
    lit = tuple(min(255, int(v * 1.16)) for v in rgb(colors[0]))
    shd = dk(colors[1], 0.62)
    ear_dk = dk(colors[1], 0.92)
    lay = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    d = ImageDraw.Draw(lay)
    ...
    for i in range(rows + 1):
        ...
        while u < u1:
            x, y = F.P(u, v)
            y += rnd.uniform(-3, 3)
            top = (x + rnd.uniform(-4, 4), y - hh * rnd.uniform(0.78, 1.04))
            d.line([(x, y), top], fill=shd + (160,), width=max(2, int(4 * s)))
            d.line([(x - 2, y - hh * 0.3), (top[0] - 2, top[1] + 6)], fill=lit + (110,), width=max(2, int(3 * s)))
            col = ear_dk if rnd.random() < 0.55 else rgb(colors[0])
            d.ellipse((top[0] - 5 * s, top[1] - 12 * s, top[0] + 5 * s, top[1] + 8 * s), fill=tuple(int(c) for c in col) + (220,))
            d.ellipse((top[0] - 4 * s, top[1] - 10 * s, top[0], top[1] - 2 * s), fill=lit + (90,))
            u += rnd.uniform(0.01, 0.02) / s
    paste_clipped(ic, lay, union(m, F.mask(ic, max(0, v0 - 0.12), v0 + 0.02, u0, u1)))
    tint(ic, F.mask(ic, v0 + (v1 - v0) * 0.45, v1, u0, u1), m, (34, 22, 8), 0.3, 16)  # darker towards the front
    tint(ic, F.mask(ic, v0, v0 + (v1 - v0) * 0.3, u0, u1), m, (255, 240, 200), 0.12, 10)  # sunlit back
    return m


def furrows(ic: Icon, F: Field, v0: float, v1: float, u0=0.0, u1=1.0, n: int = 7, soil=("#5a3e2a", "#2c1e14")) -> Image.Image:
    """Fresh ploughed land seen along the furrows: ridges running away from the viewer and converging
    to the back, a dark trench beside each, uneven brown, irregular clods."""
    rnd = ic.random
    m = F.mask(ic, v0, v1, u0, u1)
    ys = [p[1] for p in F.band(v0, v1, u0, u1)]
    ic.fill(m, soil[0], soil[1], (min(ys), max(ys)), noise=0.4, chroma=0.12)
    for _ in range(9):  # uneven, patchy earth
        u, v = rnd.uniform(u0, u1), rnd.uniform(v0, v1)
        x, y = F.P(u, v)
        r = rnd.uniform(30, 70) * F.scale(v)
        tint(ic, ic.mask("ellipse", (x - r, y - r * 0.5, x + r, y + r * 0.5)), m,
             rnd.choice([(20, 12, 6), (140, 104, 70), (70, 50, 30)]), rnd.uniform(0.15, 0.3), 12)
    lay = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    d = ImageDraw.Draw(lay)
    ...
    for k in range(n):
        u = u0 + (u1 - u0) * (k + 0.5) / n
        du = (u1 - u0) / n
        crest = [F.P(u + rnd.uniform(-0.004, 0.004), v) for v in vs]
        trench = [F.P(u + du * 0.42, v) for v in vs]
        for a, b, c, e, v in zip(crest, crest[1:], trench, trench[1:], vs):
            s = F.scale(v)
            d.polygon([a, b, e, c], fill=(18, 10, 4, 130))  # shadowed side of the ridge
            d.line([a, b], fill=(150, 116, 82, 150), width=max(2, int(7 * s)))  # lit crest
    for _ in range(int(140 * (u1 - u0))):  # clods, bigger in front
        u, v = rnd.uniform(u0, u1), rnd.uniform(v0, v1)
        x, y = F.P(u, v)
        s = F.scale(v)
        r = rnd.uniform(4, 11) * s
        pts = [(x + math.cos(t) * r * rnd.uniform(0.7, 1.2), y + math.sin(t) * r * 0.7 * rnd.uniform(0.7, 1.2))
               for t in np.linspace(0, 2 * math.pi, 7, endpoint=False)]
        d.polygon([(px + 2, py + 3) for px, py in pts], fill=(16, 10, 4, 150))
        d.polygon(pts, fill=(112, 82, 54, 220) if rnd.random() < 0.6 else (74, 52, 32, 220))
        d.ellipse((x - r * 0.7, y - r * 0.6, x, y - r * 0.05), fill=(168, 134, 96, 130))
    paste_clipped(ic, lay, m)
    return m
...
TILE = ("#a0664a", "#5a3224")
VETCH = ((74, 112, 50), (124, 160, 78), (24, 42, 18))


def farmhouse(ic: Icon) -> None:
    L, R, EAVE, BASE = 120, 440, 450, 640
    stones(ic, (L, EAVE - 10, R, BASE), "#b0a490", "#7a705e", 34)
    ic.door(250, 540, 310, BASE - 4, arched=False, surround=None)
    for x in (160, 360):
        ic.window(x, 500, x + 40, 544)
    ic.shade(ic.mask("rectangle", (L, EAVE, R, EAVE + 50)), (20, 12, 4), 0.45, blur=12)
    shingles(ic, [(L - 34, EAVE + 14), (R + 34, EAVE + 14), (R - 30, 300), (L + 30, 300)], TILE[0], TILE[1], 30, 38)
    ic.shade(ic.poly_mask([(R - 30, 300), (R + 34, EAVE + 14), (R - 60, EAVE + 14)]), (20, 10, 4), 0.3, blur=3)
    ic.line([(L + 30, 300), (R - 30, 300)], 14, (110, 64, 46))
    cm = ic.mask("rectangle", (170, 222, 214, 310))
    stones(ic, (170, 222, 214, 310), "#a89c88", "#6e6452", 22, clip=cm)
    ic.rect((164, 214, 220, 228), "#8e8272", "#5a5046", edge=4)
    ic.outline([(170, 222), (214, 222), (214, 310), (170, 310)], 4)


def barn(ic: Icon) -> None:
    rnd = ic.random
    L, R, EAVE, BASE = 480, 910, 420, 640
    planks(ic, (L, EAVE - 10, R, BASE), "#8e6c48", "#4a3622", 28)
    stones(ic, (L, BASE - 40, R, BASE), "#a0968a", "#6e665a", 26)
    # great cart doors at the right end, one half open
    ic.rect((740, 470, 870, BASE - 40), "#2a1e14", "#120c08", edge=4)
    planks(ic, (740, 470, 800, BASE - 40), "#7a5a3a", "#3e2c1a", 22)
    ic.outline([(740, 470), (800, 470), (800, BASE - 40), (740, BASE - 40)], 4)
    hay(ic, [(810, BASE - 40), (816, 560), (840, 540), (866, 548), (870, BASE - 40)], 60)
    timber(ic, (734, 466), (876, 466), 18)
    for x in (L + 10, R - 10):
        timber(ic, (x, EAVE - 6), (x + rnd.uniform(-2, 2), BASE), 22)
    ic.shade(ic.mask("rectangle", (L, EAVE, R, EAVE + 50)), (20, 12, 4), 0.45, blur=12)
    shingles(ic, [(L - 36, EAVE + 16), (R + 36, EAVE + 16), (R - 50, 252), (L + 50, 252)], TILE[0], TILE[1], 30, 40)
    ic.shade(ic.poly_mask([(R - 50, 252), (R + 36, EAVE + 16), (R - 90, EAVE + 16)]), (20, 10, 4), 0.3, blur=3)
    ic.line([(L + 50, 252), (R - 50, 252)], 14, (110, 64, 46))
    # louvred vent on the ridge
    ic.poly([(680, 256), (730, 256), (726, 214), (684, 214)], "#8a6a48", "#4a3622", edge=4)
    ic.poly([(672, 218), (738, 218), (720, 196), (690, 196)], TILE[0], TILE[1], edge=4)


def strip_clip(ic: Icon, F: Field, u0: float, u1: float, up: float = 0.9, down: float = 160) -> Image.Image:
    """The column above and below one strip, bounded by its two converging edge lines (extended)."""
    def ext(u):
        (xb, yb), (xf, yf) = F.P(u, 0), F.P(u, 1)
        top = (xb + (xb - xf) * up, yb + (yb - yf) * up)
        return top, (xf, yf), (xf, yf + down)

    t0, f0, b0 = ext(u0)
    t1, f1, b1 = ext(u1)
    return ic.poly_mask([t0, t1, f1, b1, b0, f0])
...
def draw(ic: Icon) -> None:
    ...
    barn(ic)
    farmhouse(ic)
    hedge(ic, 70, 960, 656, 66, 22)

    # the slab narrows to the back so the strips are wedges converging behind the hedge
    F = Field((210, 650), (830, 650), (16, 934), (1008, 934), persp=1.3)
    slab_face(ic, F, 66)
    U = (0.0, 0.31, 0.5, 0.69, 1.0)
    # pulses: bare soil between rows of bushes, the chain's crop, kept inside their strip
    soil(ic, F.band(0, 1, U[0], U[1]), clods=40)
    with ic.clipped(strip_clip(ic, F, U[0], U[1] + 0.04)):
        for v, n in ((0.05, 3), (0.27, 3), (0.55, 3), (0.94, 2)):
            s = F.scale(v)
            for k in range(n):
                u = U[0] + (U[1] - U[0]) * (k + 0.5) / n + rnd.uniform(-0.01, 0.01)
                x, y = F.P(u, v)
                pulse_bush(ic, x, y + 4, 222 * s * rnd.uniform(0.92, 1.04), 196 * s * rnd.uniform(0.92, 1.05),
                           pods=0 if v < 0.25 else (3 if v < 0.7 else 5), flowers=1)
    grain(ic, F, 0.0, 1.0, U[1], U[2], 92, ("#cca65a", "#7a5828"))
    furrows(ic, F, 0.0, 1.0, U[2], U[3], 5)
    # vetch: low flowering clumps in a strip as wide as the pulses
    soil(ic, F.band(0, 1, U[3], U[4]), ("#6a5236", "#3e2e1e"), clods=30)
    with ic.clipped(strip_clip(ic, F, U[3] - 0.02, U[4])):
        for v, n in ((0.08, 3), (0.3, 3), (0.56, 3), (0.84, 2)):
            s = F.scale(v)
            for k in range(n):
                u = U[3] + (U[4] - U[3]) * (k + 0.5) / n + rnd.uniform(-0.01, 0.01)
                x, y = F.P(u, v)
                pulse_bush(ic, x, y + 4, 170 * s, 100 * s, pods=0, leaf=VETCH, leaflet=10, flowers=6,
                           bloom=((150, 96, 170), (120, 70, 140)))
    for u in U[1:-1]:  # dividing balks between the strips
        ic.line([F.P(u, 0), F.P(u, 1)], 6, (56, 42, 26, 120))

    # an open sack of beans at the bottom right, standing on the slab edge
    ic.shade(ic.mask("ellipse", (800, 950, 1016, 1004)), alpha=0.5, blur=10)
    bean_sack(ic, 920, 1004, 168, 190, weights=(2, 3, 2, 3, 1), bean=12)
