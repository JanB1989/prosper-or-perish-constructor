"""Market Cotton Farm: a two-storey gin house with a big roller-gin wheel, bound bales, cotton bushes.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/market_cotton_farm/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/market_cotton_farm

Identity: the large spoked wheel of the roller gin in the open bay of a timber gin house under a
shingle roof, lint heaped on its floor, and a stack of pressed, rope-bound bales at the bottom right;
a short row of cotton bushes with white bolls in front. Tier 1 of the cotton chain: the Cotton Farm's
thatched shed and baskets become a gin house and a bale yard.
...
SEED = 37
REFS = ("cotton_plantation", "farming_village", "fiber_crops_farm", "windmill")
...
def bale(ic: Icon, cx, cy, w, h, depth: float = 36, angle: float = 0.0, cloth=("#e6dcc2", "#8c7c60"),
         bands: int = 2) -> Image.Image:
    """Soft pressed cotton bale in cream hessian, seen slightly from above: a rounded pillow of a
    block whose sides bulge and whose top sags a little, a lit top face, a fine weave, thin rope
    bands that pinch into the bulge and lint wisps squeezed out at the seams. Returns the mask."""
    rnd = ic.random
    a, b = w / 2, (h + depth) / 2
    n = 4.2
    ...
    def body_pt(t):
        ...
        x *= 1 + 0.06 * max(0.0, y / b) - 0.02 * max(0.0, -y / b)
        if y < 0:
            y += 7 * (1 - (x / a) ** 2)
        return x, y

    loc = [body_pt(t) for t in np.linspace(0, 2 * math.pi, 72, endpoint=False)]

    def P(pts):
        return ic.rotate(pts, (cx, cy), angle)

    pts = P(loc)
    m = ic.poly_mask(pts)
    ic.fill(m, cloth[0], cloth[1], radial=(cx - a * 0.7, cy - b * 0.8, max(a, b) * 2.2), noise=0.2, chroma=0.08)
    seam_y = -b + depth
    seam = [(x, seam_y + 10 * (1 - (x / a) ** 2)) for x in np.linspace(-a * 1.1, a * 1.1, 24)]
    top = ic.intersect(m, ic.poly_mask(P(seam + [(a * 1.2, -b * 1.3), (-a * 1.2, -b * 1.3)])))
    tint(ic, top, None, (255, 250, 232), 0.3, 4)
    x0, x1 = cx - a * 1.3, cx + a * 1.3
    y0, y1 = cy - b * 1.3, cy + b * 1.3

    def weave(d):  # fine crossing threads and a few coarse slubs
        for y in np.arange(y0, y1, 7):
            d.line([(x0, y), (x1, y + rnd.uniform(-2, 2))], fill=(90, 72, 44, 34), width=2)
        for x in np.arange(x0, x1, 7):
            d.line([(x, y0), (x + rnd.uniform(-2, 2), y1)], fill=(255, 248, 226, 30), width=2)
        for _ in range(int(w * h / 700)):
            x, y = rnd.uniform(x0, x1), rnd.uniform(y0, y1)
            d.line([(x, y), (x + rnd.uniform(-8, 8), y + rnd.uniform(-3, 3))], fill=(110, 90, 58, 50), width=2)

    ic.overlay(weave, m)
    # form: shadowed underside and right flank, seam shadow under the top face
    tint(ic, ic.poly_mask(P([(-a * 1.3, b * 0.35), (a * 1.3, b * 0.35), (a * 1.3, b * 1.3), (-a * 1.3, b * 1.3)])), m,
         (60, 44, 26), 0.34, 16)
    tint(ic, ic.poly_mask(P([(a * 0.55, -b * 1.3), (a * 1.3, -b * 1.3), (a * 1.3, b * 1.3), (a * 0.55, b * 1.3)])), m,
         (50, 36, 22), 0.3, 18)
    ic.line(P([(x, y + 3) for x, y in seam]), 5, (90, 70, 44, 110))
    # rope bands pinching the bulge, highlights on the bulges between them
    fr = [(i + 0.5) / bands + rnd.uniform(-0.05, 0.05) for i in range(bands)]
    xs = [-a + w * f for f in fr]
    for xa, xb in zip([-a] + xs, xs + [a]):
        bl = P([((xa + xb) / 2, seam_y + (b - seam_y) * 0.45)])[0]
        rw, rh = (xb - xa) * 0.32, (b - seam_y) * 0.28
        tint(ic, ic.mask("ellipse", (bl[0] - rw, bl[1] - rh, bl[0] + rw, bl[1] + rh)), m, (255, 250, 236), 0.2, 10)
    for bx in xs:
        path = [(bx * (1 + 0.06 * max(0.0, y / b)), y) for y in np.linspace(-b - 4, b + 4, 16)]
        pth = P(path)
        pinch = blank()
        ImageDraw.Draw(pinch).line(pth, fill=255, width=20)
        tint(ic, pinch, m, (40, 28, 14), 0.4, 6)
        with ic.clipped(m):
            ic.line(pth, 6, (86, 64, 38))
            ic.line([(x - 2, y) for x, y in pth], 2, (196, 164, 116, 200))
    ic.outline(pts, 4, (40, 26, 16, 170))
    # lint wisps squeezed out at the seam and the rope crossings
    spots = [(rnd.uniform(-a * 0.7, a * 0.1), seam_y + rnd.uniform(0, 8))]
    spots += [(a * 0.97, rnd.uniform(-b * 0.1, b * 0.4))]

    def wisps(d):
        for x, y in P(spots):
            for _ in range(3):
                r = rnd.uniform(7, 12)
                px, py = x + rnd.uniform(-10, 10), y + rnd.uniform(-5, 5)
                d.ellipse((px - r + 2, py - r * 0.6 + 3, px + r + 2, py + r * 0.6 + 3), fill=(150, 146, 138, 170))
                d.ellipse((px - r, py - r * 0.6, px + r, py + r * 0.6), fill=(244, 242, 234, 240))
            for _ in range(4):
                q = rnd.uniform(0, 2 * math.pi)
                d.line([(x, y), (x + math.cos(q) * 20, y + math.sin(q) * 12)], fill=(246, 244, 236, 180), width=2)
...
def gin(ic: Icon, x0, x1, y, fx, fy, fr, clip) -> None:
    """Hand roller gin: a pair of horizontal rollers on a bench, a small flywheel with a crank handle
    at their right end, and a bright fall of lint pouring from the nip onto the floor."""
    timber(ic, (x0 - 10, y + 30), (x1 + 10, y + 30), 20)  # bench rail
    for x in (x0 + 6, x1 - 6):
        timber(ic, (x, y - 30), (x, y + 150), 16)
    for yy in (y - 12, y + 10):  # the paired rollers
        ic.line([(x0, yy), (x1, yy)], 18, (34, 34, 38))
        ic.line([(x0, yy - 5), (x1, yy - 5)], 5, (160, 160, 158, 190))
    # lint pouring from the nip: a widening bright sheet with soft strands
    rnd = ic.random
    mid = (x0 + x1) / 2
    puffs = []
    for k in range(34):  # a tumbling column of fluffy lint, widening as it falls
        t = rnd.random() ** 0.8
        half = (x1 - x0) * (0.3 + 0.22 * t)
        puffs.append((mid - 8 + rnd.uniform(-half, half) * (1 - 0.2 * rnd.random()), y + 26 + t * 100, rnd.uniform(14, 24)))
    puffs.sort(key=lambda p: p[1])
    fm = blank()
    fd = ImageDraw.Draw(fm)
    for px, py, r in puffs:
        fd.ellipse((px - r, py - r * 0.85, px + r, py + r * 0.85), fill=255)
    fm = ic.intersect(fm, clip)
    ic.fill(fm, "#f6f2e6", "#c4c4c4", (y + 22, y + 130), noise=0.1, chroma=0.04)

    def paint(d):
        for px, py, r in puffs:
            lit = max(0.0, min(1.0, 0.6 - (px - mid) / (x1 - x0)))
            d.ellipse((px - r + 4, py - r * 0.8 + 5, px + r + 4, py + r * 0.8 + 5), fill=(128, 130, 142, 170))
            d.ellipse((px - r, py - r * 0.8, px + r, py + r * 0.8), fill=(int(214 + 34 * lit), int(212 + 32 * lit), int(204 + 26 * lit), 240))
            d.ellipse((px - r * 0.7, py - r * 0.7, px + r * 0.3, py), fill=(255, 252, 240, 190))

    ic.overlay(paint, fm)
    tint(ic, ic.mask("rectangle", (x0 - 20, y + 12, x1 + 20, y + 40)), fm, (40, 40, 50), 0.3, 8)
    # flywheel and crank
    ic.line([(x1, y - 1), (fx, fy)], 10, (60, 44, 28))  # axle
    wheel(ic, fx, fy, fr, spokes=6, rim=12)
    hx, hy = fx + fr * 0.62, fy + fr * 0.55
    ic.line([(fx, fy), (hx, hy)], 12, (58, 40, 24))
    ic.line([(hx, hy), (hx + 26, hy + 8)], 12, (150, 112, 70))
    ic.line([(hx + 2, hy - 3), (hx + 24, hy + 4)], 4, (220, 186, 130, 200))
...
def draw(ic: Icon) -> None:
    ...
    L, R, EAVE, FLOOR, BASE = 70, 620, 420, 610, 840
    BAY = 320  # open gin bay from BAY to R

    # ---- screw bale press behind the bale yard: posts, head beam, great screw, capstan bar, box
    PX0, PX1 = 700, 880
    for x in (PX0, PX1):
        timber(ic, (x, 470), (x, 880), 30, "#8e6844", "#4e3522")
    timber(ic, (PX0 - 30, 486), (PX1 + 30, 486), 34, "#94704a", "#50361f")
    cx = (PX0 + PX1) / 2
    ic.rect((cx - 18, 380, cx + 18, 640), "#9a7a52", "#5a4028", vertical=False, edge=4)
    for y in np.arange(392, 640, 22):  # screw thread
        ic.line([(cx - 18, y), (cx + 18, y + 12)], 6, (60, 40, 22))
        ic.line([(cx - 16, y - 4), (cx + 16, y + 8)], 3, (200, 170, 120, 150))
    timber(ic, (cx - 150, 404), (cx + 150, 396), 18, "#9a7650", "#56391f")
    ic.rect((PX0 + 16, 640, PX1 - 16, 670), "#86603e", "#4e3522", edge=4)  # platen
    planks(ic, [(PX0 + 16, 670), (PX1 - 16, 670), (PX1 - 16, 880), (PX0 + 16, 880)], "#7e5a3a", "#42301c", board=26)
    ic.outline([(PX0 + 16, 670), (PX1 - 16, 670), (PX1 - 16, 880), (PX0 + 16, 880)], 5)

    # ---- upper storey: boards with a loft door; ground floor: warmer boards on a stone plinth, open bay
    planks(ic, [(L, EAVE), (R, EAVE), (R, FLOOR), (L, FLOOR)], "#8a6442", "#50361f", board=30)
    planks(ic, [(L, FLOOR), (BAY, FLOOR), (BAY, BASE), (L, BASE)], "#b0703e", "#5e321a", board=30)
    ic.rect((140, 668, 240, BASE - 60), "#6e4a2c", "#3e2816", edge=4)  # plank door
    for y in (700, 750):
        ic.line([(144, y), (236, y)], 8, (48, 36, 26))
    ic.window(430, 480, 494, 544)
    ic.rect((200, 460, 290, FLOOR - 4), "#2c2016", "#140e09", edge=4)  # open loft door
    lint_heap(ic, 245, 594, 48, 40, ic.mask("rectangle", (200, 460, 290, FLOOR - 4)), n=16)

    # ---- the open gin bay: dark interior, the roller gin up and right, lint pouring onto the floor
    ic.rect((BAY, FLOOR, R, BASE), "#33261a", "#140e09", edge=0)
    bay = ic.mask("rectangle", (BAY, FLOOR, R, BASE))
    ic.shade(ic.mask("rectangle", (BAY, FLOOR, R, FLOOR + 70)), alpha=0.55, blur=14)
    gin(ic, 380, 540, 668, 552, 660, 50, bay)
    lint_heap(ic, 460, BASE - 6, 130, 58, bay, n=28)

    # ---- stone plinth under the ground floor and the bay sill
    ic.stonewall((L - 10, BASE - 60, BAY + 10, BASE + 10))
    tint(ic, ic.mask("rectangle", (L - 10, BASE - 60, BAY + 10, BASE - 30)), None, (20, 12, 6), 0.3, 8)
    ic.stonewall((BAY - 10, BASE - 10, R + 10, BASE + 14))

    # ---- frame: corner posts, floor beam, braces
    for x in (L + 8, BAY, R - 8):
        timber(ic, (x, EAVE), (x, BASE - 58), 26)
    timber(ic, (L, FLOOR), (R, FLOOR), 24)
    timber(ic, (L + 20, FLOOR - 8), (L + 100, EAVE + 16), 16)
    timber(ic, (R - 20, FLOOR - 8), (R - 100, EAVE + 16), 16)
    ic.shade(ic.mask("rectangle", (L, EAVE, R, EAVE + 60)), alpha=0.5, blur=14)
    # the gable (right) end of the house in shade
    tint(ic, ic.mask("rectangle", (R - 150, EAVE, R + 20, BASE)), ic.mask("rectangle", (L - 20, EAVE, R + 20, BASE + 20)),
         (20, 12, 6), 0.34, 30)

    # ---- shingle roof: sunlit front plane, hip ends, the right one in shade
    roof = [(L - 44, EAVE + 14), (R + 44, EAVE + 14), (R - 30, 230), (L + 30, 230)]
    rm = shingles(ic, roof, "#c09870", "#6a4a2c", course=30, stagger=40)
    tint(ic, ic.mask("ellipse", (L - 40, 160, R - 120, EAVE - 20)), rm, (255, 232, 196), 0.2, 50)
    tint(ic, ic.poly_mask([(R - 30, 230), (R + 44, EAVE + 14), (R - 70, EAVE + 14)]), rm, (26, 14, 6), 0.55, 6)
    tint(ic, ic.poly_mask([(L + 30, 230), (L - 44, EAVE + 14), (L + 60, EAVE + 14)]), rm, (255, 236, 200), 0.14, 6)
    ic.line([(R - 30, 230), (R - 70, EAVE + 14)], 5, (40, 24, 12, 150))
    ic.line([(L + 30, 230), (L + 60, EAVE + 14)], 5, (40, 24, 12, 120))
    ic.rect((L + 20, 214, R - 20, 234), "#9a7c5e", "#5e4630")
    ic.line([(L + 24, 217), (R - 24, 217)], 4, (230, 204, 164, 170))

    # ---- cotton bushes in front of the house, on low field earth
    earth = earth_band(ic, 0, 380, 900, 985, furrows=(946,))
    for cx, h, w in ((80, 300, 190), (235, 320, 190), (352, 230, 150)):
        ic.shade(ic.intersect(ic.mask("ellipse", (cx - w * 0.5, 966, cx + w * 0.55, 1004)), earth), (14, 10, 4), 0.55, blur=10)
        cotton_bush(ic, cx + rnd.uniform(-8, 8), 990, h, w, density=0.75, boll_r=25)

    # ---- bale yard, bottom right: three soft bales overlapping irregularly, a basket of lint
    ic.shade(ic.mask("ellipse", (520, 950, 1020, 1014)), alpha=0.45, blur=10)
    bale(ic, 900, 818, 210, 108, depth=40, angle=-4, cloth=("#f2dcac", "#9a7a4c"))
    bale(ic, 700, 940, 220, 112, depth=40, angle=3, cloth=("#ead2a2", "#90724a"))
    bale(ic, 910, 948, 196, 104, depth=38, angle=-2, cloth=("#f4e0b4", "#967850"))
    basket(ic, 500, 918, 590, 1006, ("#a8804c", "#584024"))
    lint_heap(ic, 545, 926, 46, 30, n=22)
    ic.overlay(lambda d: d.arc((500, 906, 590, 930), 10, 170, fill=(170, 130, 80, 255), width=9))
