"""Rice Paddy (rice_farm): small flooded paddies between curving earth bunds, a thatched field shelter.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/rice_farm/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/rice_farm

Identity: a block of shallow, sky-bright water cut into small irregular paddies by low grassy
bunds, set with rows of bright green transplanted rice hills; one paddy is still open water with
seedling bundles thrown in for planting. A small thatched field shelter on stilts stands at the back
left, a basket of seedlings on the front bund at the bottom right. Tier 1 of the rice chain: no
house, no stores, hand work only.

Family (rice): shares the helpers and palette below with rice_farmstead, rice_rotations and
...
def seed_basket(ic: Icon, x0, y0, x1, y1) -> None:
    """Shallow carrying basket heaped with seedling bundles: dark inside, warm light rim, the
    seedling tops standing well clear of the rim."""
    ic.shade(ic.mask("ellipse", (x0 - 10, y1 - 12, x1 + 20, y1 + 14)), alpha=0.45, blur=8)
    R = ic.random
    ic.basket(x0, y0, x1, y1, colors=("#c49a5c", "#6c4e2c"))
    ic.fill(ic.mask("ellipse", (x0 + 2, y0 - 16, x1 - 2, y0 + 14)), "#2e2012", "#4a3420", (y0 - 16, y0 + 14), noise=0.2)
    lay, d = layer()
    for k in range(22):
        bx = R.uniform(x0 + 12, x1 - 12)
        a = R.uniform(-0.55, 0.55)
        L = R.uniform(62, 96)
        col = YOUNG[k % 3]
        d.line([(bx, y0 - 5), (bx + math.sin(a) * L, y0 - 5 - math.cos(a) * L)], fill=col + (255,), width=7)
    composite(ic, lay)
    ic.overlay(lambda dr: dr.arc((x0 + 2, y0 - 16, x1 - 2, y0 + 14), 190, 350, fill=(176, 134, 78, 255), width=6))
    ic.overlay(lambda dr: dr.arc((x0 - 2, y0 - 18, x1 + 2, y0 + 18), 0, 180, fill=(240, 202, 130, 255), width=15))
    ic.overlay(lambda dr: dr.arc((x0, y0 - 12, x1, y0 + 20), 20, 160, fill=(92, 62, 30, 160), width=3))
...
def limb(ic: Icon, p0, p1, w0: float, w1: float) -> None:
    """Tapering bough from p0 (width w0) to p1 (width w1), bark lit on the upper left."""
    (x0, y0), (x1, y1) = p0, p1
    L = math.hypot(x1 - x0, y1 - y0)
    nx, ny = -(y1 - y0) / L, (x1 - x0) / L
    ...
    xs = [p[0] for p in pts]
    ic.fill(ic.poly_mask(pts), "#8e7862", "#3a2c22", (min(xs), max(xs)), vertical=False, noise=0.26)


def tree(ic: Icon, cx: float, base: float, h: float, r: float, color=("#5f8a3a", "#1e3212")) -> Image.Image:
    """Leafy tree: a trunk forking into two boughs under a crown of three or four overlapping
    lobes, each lit from the upper left with a shaded underside, dark shadow gaps where one lobe
    lies over the next and a ragged lower edge. Returns the crown mask."""
    R = ic.random
    cy = base - h
    fork = base - h * 0.52
    trunk = [(cx - 34, base), (cx - 19, base - h * 0.3), (cx - 16, fork), (cx + 16, fork),
             (cx + 20, base - h * 0.3), (cx + 38, base)]
    ic.poly(trunk, "#8e7862", "#3a2c22", span=(cx - 22, cx + 24), vertical=False, noise=0.26, edge=3)
    # the fork: two main boughs and a thinner leader into the crown
    limb(ic, (cx - 6, fork + 10), (cx - r * 0.46, cy + r * 0.10), 26, 12)
    limb(ic, (cx + 6, fork + 10), (cx + r * 0.44, cy + r * 0.12), 24, 11)
    limb(ic, (cx + 2, fork + 6), (cx + r * 0.06, cy - r * 0.10), 16, 8)
    ic.shade(ic.mask("ellipse", (cx - 24, fork - 10, cx + 24, fork + 24)), (20, 14, 8), 0.35, blur=6)
    # lobes back to front: (centre x, centre y, radius, tone)
    lobes = [(cx + r * 0.08, cy - r * 0.34, r * 0.58, 0.80),
             (cx + r * 0.60, cy + r * 0.02, r * 0.50, 0.92),
             (cx - r * 0.56, cy - r * 0.04, r * 0.54, 1.0),
             (cx + r * 0.10, cy + r * 0.02, r * 0.42, 1.06)]
    m = blank()
    for lx, ly, lr, tone in lobes:
        lm = blank()
        blobs = []
        for _ in range(11):
            a, d = R.uniform(0, 2 * math.pi), R.random() ** 0.6
            br = lr * R.uniform(0.34, 0.52)
            by = ly + math.sin(a) * lr * 0.52 * d
            if by > ly:  # ragged lower edge: some blobs sag, some tuck in
                by += R.uniform(-0.12, 0.22) * lr
            blobs.append((lx + math.cos(a) * lr * 0.72 * d, by, br))
        for bx, by, br in blobs:
            lm = union(lm, ic.poly_mask(ic.jitter(bx, by, br * 1.12, br * 0.92, 20, 0.14)))
        lm = lm.filter(ImageFilter.MaxFilter(11)).filter(ImageFilter.MinFilter(11))  # close pinholes
        # shadow this lobe throws on the crown behind it: the dark gap between lobes
        if m.getbbox():
            ic.shade(ic.intersect(m, ImageChops.offset(lm, 12, 18)), (6, 12, 4), 0.80, blur=8)
        c1, c2 = scale_c(color[0], tone), scale_c(color[1], tone)
        # the lobe as one rounded form: lit upper left, dark lower right
        ic.fill(lm, c1, c2, radial=(lx - lr * 0.55, ly - lr * 0.6, lr * 1.9), noise=0.3, chroma=0.14)
        blobs.sort(key=lambda b: b[1])
        for bx, by, br in blobs:  # leaf clumps inside the lobe, each with a soft lit cap
            if by < ly + lr * 0.15:
                cap = ic.mask("ellipse", (bx - br * 0.9, by - br * 0.8, bx + br * 0.3, by + br * 0.1))
                ic.shade(ic.intersect(lm, cap), (214, 230, 150), 0.16, blur=10)
        ic.shade(ic.intersect(lm, ic.mask("rectangle", (0, ly + lr * 0.18, SIZE, SIZE))), (10, 18, 6), 0.40, blur=14)
        # dark seam where this lobe lies over the crown behind it
        if m.getbbox():
            ring = ImageChops.subtract(lm.filter(ImageFilter.MaxFilter(13)), lm)
            ic.shade(ic.intersect(m, ring), (6, 12, 4), 0.75, blur=3)
        m = union(m, lm)
...
SEED = 61
REFS = ("polders", "farming_village", "terraces", "irrigation_systems")


def shelter(ic: Icon) -> None:
    """Open field shelter: raised plank floor on stilts, woven back screen, thatched roof."""
    x0, x1 = 172, 436
    floor, foot, top = 368, 476, 228
    ic.shade(ic.mask("ellipse", (x0 - 10, foot - 20, x1 + 50, foot + 18)), (10, 30, 30), 0.45, blur=10)
    ic.rect((x0 + 8, top, x1 - 8, floor), "#3a2c1c", "#1e160e", edge=0)
    lay, d = layer()
    for y in np.arange(top + 6, floor, 18):
        d.line([(x0 + 10, y), (x1 - 10, y + 2)], fill=(120, 96, 60, 90), width=6)
    for x in np.arange(x0 + 16, x1 - 8, 24):
        d.line([(x, top + 4), (x + 2, floor)], fill=(90, 70, 40, 70), width=4)
    composite(ic, lay, ic.mask("rectangle", (x0 + 8, top, x1 - 8, floor)))
    ic.shade(ic.mask("rectangle", (x0 + 8, top, x1 - 8, top + 60)), alpha=0.5, blur=10)
    # clay water jar and a rolled mat on the floor
    ic.fill(ic.mask("ellipse", (344, 304, 404, 370)), "#b0784c", "#5a3620", radial=(352, 314, 72), noise=0.2)
    ic.fill(ic.mask("ellipse", (358, 296, 390, 310)), "#3a2a1e", "#20160e", noise=0.1)
    ic.fill(ic.mask("rounded_rectangle", (214, 336, 320, 366), radius=14), "#c4a468", "#7a5e30", noise=0.25)
    for x in (236, 262, 290):
        ic.line([(x, 338), (x, 364)], 3, (80, 60, 30, 150))
    ic.rect((x0 - 8, floor, x1 + 8, floor + 20), "#9a7450", "#5e4028", edge=4)
    ic.shade(ic.mask("rectangle", (x0 - 8, floor + 20, x1 + 8, floor + 38)), alpha=0.5, blur=6)
    for x in (x0 + 4, (x0 + x1) / 2 - 10, x1 - 4):
        post(ic, x + ic.random.uniform(-3, 3), top - 12, foot + ic.random.uniform(-4, 4), 20)
    ridge = [(240, 108), (304, 100), (368, 106)]
    eave = [(x, 262 + ic.random.uniform(-4, 5)) for x in np.linspace(118, 494, 16)]
    thatch(ic, ridge, eave, courses=(0.0, 0.3, 0.55, 0.78, 1.0))
    ic.shade(ic.poly_mask([(340, 90), (520, 90), (520, 300), (380, 300)]), alpha=0.18, blur=30)
...
def draw(ic: Icon) -> None:
    ...
    blk = Block(150, 880, 450, 30, 994, 800)
    hb = [const(0.0), wavy(0.36, 0.03, 1.2, 0.5), wavy(0.68, 0.025, 1.0, 2.2), const(1.0)]
    vs = [[0.55], [0.30, 0.70], [0.52]]
    crops = [["grown", "young"], ["young", None, "young"], ["grown", "young"]]
    def back():
        tree(ic, 790, 456, 160, 170)
        shelter(ic)

    paddies(ic, blk, hb, vs, crops, bw=24, du=0.052, hooks={0: back})

    # seedling bundles thrown into the open paddy, ready for transplanting
    for u, v in ((0.40, 0.46), (0.47, 0.55), (0.56, 0.47), (0.61, 0.59), (0.44, 0.63)):
        x, y = blk.P(u + R.uniform(-0.01, 0.01), v)
        s = blk.s(v)
        lay, d = layer()
        d.ellipse((x - 26 * s, y - 6 * s, x + 26 * s, y + 10 * s), fill=(20, 44, 36, 90))
        composite(ic, lay)
        seedlings(ic, x, y, 0.8 * s)

    # basket of seedlings on the front bund, bottom right
    seed_basket(ic, 812, 742, 944, 846)
    seedlings(ic, 760, 804, 0.9)
    # light from the upper left over the whole block
