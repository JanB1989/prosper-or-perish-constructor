"""Rice Paddy Estate (rice_rotations): two levels of paddies, early and late crop side by side, a
treadle chain pump lifting water to the upper level, a tile-roofed estate house.

Build (from eu5-building-pipeline):
  EU5_VANILLA_DIR=~/.cache/eu5-vanilla/game uv run eu5-building icon build \
    --script ../ProsperOrPerishConstructor/assets/icon_drawings/rice_rotations/draw.py \
    --out ../ProsperOrPerishConstructor/artifacts/icons/rice_rotations

Identity: a larger paddy estate on two levels. On the upper level a golden paddy of ripe rice
(the late crop, sheaves stooked on its bund) lies beside a freshly planted green paddy (the early
crop): two crops a year on the same land. A long wooden treadle chain pump runs diagonally up the
terrace face from the flooded lower paddies and spills into the upper level; its treadle axle sits
in a frame with a handrail. A tile-roofed estate house stands at the back left. Tier 3 of the rice
chain: more land, water lifted by machine, better buildings.
...
SEED = 71
REFS = ("terraces", "polders", "irrigation_systems", "farming_village")

PLASTER, PLASTER_DK = "#cbb48c", "#8e7650"


def estate_house(ic: Icon) -> None:
    """Estate house: stone plinth, plastered walls with timber posts, tiled hip roof, a side shed."""
    R = ic.random
    x0, x1, top, base = 196, 560, 268, 414
    # thatched side shed on the right, behind the house corner
    ic.rect((540, 318, 668, 418), "#9a7450", "#5a3c22", edge=4)
    lay, d = layer()
    for x in np.arange(552, 668, 22):
        d.line([(x, 320), (x, 416)], fill=(50, 32, 18, 140), width=4)
    composite(ic, lay, ic.mask("rectangle", (540, 318, 668, 418)))
    ic.shade(ic.mask("rectangle", (540, 318, 668, 350)), alpha=0.5, blur=8)
    thatch(ic, [(566, 250), (646, 246)], [(x, 332 + R.uniform(-3, 4)) for x in np.linspace(526, 690, 8)],
           courses=(0.0, 0.5, 1.0))
    # house body
    ic.rect((x0, top, x1, base), PLASTER, PLASTER_DK, noise=0.2, edge=5)
    ic.stonewall((x0 - 6, base - 34, x1 + 6, base + 4), "#a09482", "#6e6454", course=19)
    for x in (x0 + 8, 318, 438, x1 - 8):
        post(ic, x, top, base - 34, 20, "#7a5434", "#402a18")
    ic.door(352, 324, 404, base - 34, arched=False, surround=None)
    ic.window(232, 318, 282, 356)
    ic.window(474, 318, 524, 356)
    ic.shade(ic.mask("rectangle", (x0, top, x1, top + 56)), alpha=0.5, blur=12)
    # tiled hip roof with a heavy ridge
    roof = [(150, 290), (606, 290), (520, 150), (236, 150)]
    ic.tiles(roof, "#8a6a58", "#4a3430", course=26, stagger=30)
    ic.shade(ic.poly_mask([(150, 276), (606, 276), (606, 292), (150, 292)]), (255, 230, 200), 0.18)
    ic.rect((226, 136, 530, 158), "#6e524a", "#3e2c28", edge=4)
    for x in (226, 530):  # upturned ridge ends
        ic.poly([(x - 18, 126), (x + 6, 140), (x + 6, 158), (x - 12, 152)], "#6e524a", "#3e2c28", edge=4)
    ic.shade(ic.poly_mask([(420, 120), (640, 120), (640, 300), (470, 300)]), alpha=0.18, blur=34)


def chain_pump(ic: Icon, p0, p1) -> None:
    """Treadle chain pump: long open trough on the slope, chain of paddle boards, treadle axle and
    handrail frame at the top, water spilling out into the upper paddy."""
    (x0, y0), (x1, y1) = p0, p1
    L = math.hypot(x1 - x0, y1 - y0)
    dx, dy = (x1 - x0) / L, (y1 - y0) / L
    nx, ny = dy, -dx  # normal pointing up-left
    w = 46
    # shadow on the slope
    ic.shade(ic.poly_mask([(x0 + 20, y0 + 20), (x1 + 20, y1 + 20), (x1 + 50, y1 + 40), (x0 + 50, y0 + 40)]), alpha=0.4, blur=12)
    # trough: far side wall, water floor, near side wall
    far = [(x0 + nx * w, y0 + ny * w), (x1 + nx * w, y1 + ny * w), (x1, y1), (x0, y0)]
    ic.poly(far, "#6e4c30", "#3e2a18", edge=4)
    wat = [(x0 + nx * w * 0.7, y0 + ny * w * 0.7), (x1 + nx * w * 0.7, y1 + ny * w * 0.7), (x1 + nx * 8, y1 + ny * 8), (x0 + nx * 8, y0 + ny * 8)]
    ic.fill(ic.poly_mask(wat), "#9ccac6", "#3a7078", noise=0.1)
    # paddle boards riding up the trough
    for t in np.linspace(0.06, 0.94, 12):
        cx, cy = x0 + (x1 - x0) * t, y0 + (y1 - y0) * t
        a = (cx + nx * w * 0.95, cy + ny * w * 0.95)
        b = (cx + nx * 2, cy + ny * 2)
        ic.line([a, b], 9, (150, 110, 70))
        ic.line([(a[0] + 4, a[1] + 3), (b[0] + 4, b[1] + 3)], 4, (50, 34, 20, 160))
    timber(ic, (x0 - nx * 4, y0 - ny * 4), (x1 - nx * 4, y1 - ny * 4), 20, "#9a7450", "#5a3c24")  # near wall
    timber(ic, (x0 + nx * w, y0 + ny * w), (x1 + nx * w, y1 + ny * w), 12, "#8a6644", "#4e3624")  # far rail
    # lower sprocket drum in the water
    ic.fill(ic.mask("ellipse", (x0 - 30, y0 - 20, x0 + 30, y0 + 24)), "#7a5636", "#3a2616", noise=0.2)
    # frame at the top: two posts, handrail, treadle axle with pedal spokes
    ax, ay = x1 + nx * w * 0.4, y1 + ny * w * 0.4 - 10
    fl, fr = ax - 110, ax + 110
    for fx in (fl, fr):
        post(ic, fx, ay - 150, ay + 26, 26)
    timber(ic, (fl - 22, ay - 138), (fr + 22, ay - 142), 22)
    ic.shade(ic.poly_mask([(fl, ay + 20), (fr, ay + 20), (fr + 30, ay + 50), (fl + 30, ay + 50)]), alpha=0.35, blur=10)
    ic.fill(ic.mask("ellipse", (ax - 44, ay - 40, ax + 44, ay + 40)), "#7a5636", "#3a2616", noise=0.2)  # sprocket
    ic.overlay(lambda d: d.ellipse((ax - 44, ay - 40, ax + 44, ay + 40), outline=(40, 26, 14, 200), width=5))
    ic.rect((fl + 8, ay - 20, fr - 8, ay + 20), "#a07650", "#5a3c22", edge=5)  # axle
    ic.shade(ic.mask("rectangle", (fl + 8, ay - 20, fr - 8, ay - 10)), (255, 236, 200), 0.3)
    for k, px in enumerate((fl + 32, fl + 66, fr - 66, fr - 32)):  # treadle pegs with pedal blocks
        ang = k * 1.3
        for a in (ang, ang + math.pi / 2):
            ex, ey = math.cos(a) * 10, math.sin(a) * 44
            ic.line([(px - ex, ay - ey), (px + ex, ay + ey)], 12, (128, 92, 56))
            for sg in (-1, 1):
                qx, qy = px + sg * ex, ay + sg * ey
                ic.fill(ic.mask("ellipse", (qx - 12, qy - 9, qx + 12, qy + 9)), "#c09a68", "#6a4a2a", noise=0.2)
    # water spilling out over the top into the upper paddy
    sp = [(x1 - 6, y1 - 30), (x1 + 16, y1 - 36), (x1 + 34, y1 - 12), (x1 + 12, y1 - 6)]
    ic.overlay(lambda d: d.polygon(sp, fill=(150, 200, 206, 220)))
    ic.line([(x1, y1 - 28), (x1 + 22, y1 - 12)], 4, (240, 250, 250, 200))
...
def draw(ic: Icon) -> None:
    ...
    # ---- upper level: estate house, ripe late crop beside the young early crop
    up = Block(190, 850, 420, 110, 920, 608)
    up.k = 0.86
    paddies(ic, up, [const(0.0), const(1.0)], [[0.52]], [["ripe", "young"]], bw=22, du=0.05, dv=0.12,
            face=76, hooks={0: lambda: estate_house(ic)})
    # stooks of sheaves on the bund in front of the ripe paddy
    for x, h in ((190, 150), (330, 140)):
        stook(ic, x, 622, h, 110)

    # ---- lower level: flooded paddies in two bands
    low = Block(50, 972, 682, 20, 1004, 878)
    hb = [const(0.0), wavy(0.52, 0.02, 1.0, 0.7), const(1.0)]
    paddies(ic, low, hb, [[0.38], [0.62]], [["grown", "young"], ["young", "grown"]], bw=24, du=0.05, dv=0.1, face=70)

    # ---- chain pump from the lower paddy up the terrace face
    chain_pump(ic, (604, 770), (790, 602))
    # a basket of seedlings bottom left on the front bund
    seed_basket(ic, 70, 800, 190, 892)
    ic.shade(ic.poly_mask([(0, 0), (SIZE, 0), (SIZE, 460), (0, 600)]), (255, 244, 214), 0.06, blur=40)
