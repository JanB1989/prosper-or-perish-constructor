...
function Invoke-LocationPotentialLocalizationInjection {
    ...
    if (-not (Test-Path -LiteralPath $modifiersPath -PathType Leaf)) {
        throw "Cannot inject Location Potential localization; missing generated modifiers file: $modifiersPath"
    }

    $locationModifiers = [System.Collections.Generic.List[string]]::new()
    $seenLocationModifiers = @{}
    Get-Content -LiteralPath $modifiersPath | ForEach-Object {
        $match = [regex]::Match($_, '^\s*(pp_loc_\S+)\s*=\s*\{\s*$')
        if ($match.Success) {
            $modifierKey = $match.Groups[1].Value
            if (-not $seenLocationModifiers.ContainsKey($modifierKey)) {
                $seenLocationModifiers[$modifierKey] = $true
                $locationModifiers.Add($modifierKey)
            }
        }
    }

    if ($locationModifiers.Count -eq 0) {
        throw "Cannot inject Location Potential localization; no pp_loc_* modifiers found in $modifiersPath"
    }

    $modifierLocalizationLines = [System.Collections.Generic.List[string]]::new()
    $modifierLocalizationLines.Add("  # Start generated location potential modifier help")
    $modifierLocalizationLines.Add('  pp_location_potential_modifier_name: "[pp_location_potential|e]"')
    $modifierLocalizationLines.Add('  pp_location_potential_modifier_desc: "Population capacity and local output modifiers for this location are calculated from [pp_location_potential|e]."')
    foreach ($modifierKey in $locationModifiers) {
        $modifierLocalizationLines.Add(('  STATIC_MODIFIER_NAME_' + $modifierKey + ': "$pp_location_potential_modifier_name$"'))
        $modifierLocalizationLines.Add(('  STATIC_MODIFIER_DESC_' + $modifierKey + ': "$pp_location_potential_modifier_desc$"'))
    }
    $modifierLocalizationLines.Add("  # End generated location potential modifier help")
    ...
    Upsert-GeneratedLocalizationBlock `
        ...
        -KeysToRemove @(
            "pp_location_modifiers_title",
            "pp_location_modifiers_title_desc",
            "pp_location_potential_modifier_name",
            "pp_location_potential_modifier_desc"
        ) `
        -Lines $modifierLocalizationLines
    ...
    Write-Host "Injected shared Location Potential localization for $($locationModifiers.Count) location modifiers."
}
