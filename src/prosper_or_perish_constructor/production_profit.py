    ...
    *,
    ...
    for building in targets:
        ...
        if current is None or not isinstance(current.value, CList):
            continue
        converted_block, slots, method_count = _converted_building_block(
            building,
            current.value,
...
def _converted_building_block(
    ...
    *,
...
) -> tuple[CList, list[dict[str, Any]], int]:
    location = _location()
    ...
    return CList(entries=entries, items=list(block.items)), slots, sum(len(slot["methods"]) for slot in slots)
...
def _localization_entries(building: str, slots: Sequence[Mapping[str, Any]]) -> dict[str, str]:
    entries: dict[str, str] = {}
    for index, slot in enumerate(slots):
        entries[f"{building}_slot_{index}"] = f"{_title_from_key(building)} Work"
        for method in slot.get("methods") or []:
            entries[str(method)] = _title_from_key(str(method).removeprefix("pp_"))
    return entries
