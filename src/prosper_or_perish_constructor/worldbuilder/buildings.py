...
OTHER_CAPACITY_BLUEPRINTS = (
    "terraces", "irrigation_reservoirs", "khmer_baray", "aqueduct_system", "pound_lock_canal_infrastructure",
)
...
def cap_script_value(contract: Contract, key: str, equation: Mapping[str, object], scale: float, level_limit: int) -> str:
    base = float(equation["base_levels"]) * scale
    lines = [f"pp_wb_cap_{key} = {{", "\tadd = {", '\t\tdesc = "BUILDING_LEVEL_WB_BASE"', f"\t\tvalue = {_fmt(base)}", "\t}"]
...
def write_caps(contract: Contract, cfg: WorldBuilderConfig, mod_root: Path) -> dict[str, dict[str, float]]:
    """Write the cap script values; return per building key {unit_people, unit_units, scale, limit}."""
    blocks: list[str] = []
    ...
    text = "\n\n".join([GENERATED, "# Improvement building level caps: base + attribute class terms + levels per development point x development.", *blocks]) + "\n"
...
def patch_improvement_blueprints(contract: Contract, cfg: WorldBuilderConfig, repo: Path, caps: Mapping[str, Mapping[str, float]]) -> list[str]:
    ...
    for key, info in caps.items():
        ...
        body = _replace_raw_modifier(body, {"local_population_capacity": _fmt(float(info["unit_units"]))}, drop_prefixes=("farm_capacity_from_",))
        body = _set_location_potential(body, gate_trigger(contract, gates[kind]))
...
def write_setup(contract: Contract, cfg: WorldBuilderConfig, caps: Mapping[str, Mapping[str, float]], owners: Mapping[str, str], mod_root: Path) -> dict[str, int]:
    ...
    rows: list[str] = []
    skipped = 0
    kinds = {str(v["kind"]): k for k, v in caps.items()}
