...
DEVELOPMENT_PATH = Path("main_menu/common/static_modifiers/pp_wb_development.txt")   # sorts after the hand-authored TRY_REPLACE of development
...
def write_static_modifiers(contract: Contract, cfg: WorldBuilderConfig, mod_root: Path, vanilla_root: Path, rgo_by_location: Mapping[str, str] | None = None) -> dict[str, object]:
    ...
    # flat capacity per development point (the handover's known term); TRY_INJECT after the hand-authored TRY_REPLACE
    k = contract.people_per_development_point
    dev_path = mod_root / DEVELOPMENT_PATH
    if k:
        dev_path.write_text("\ufeff" + "\n\n".join([GENERATED, f"# {_fmt(k)} people of population capacity per development point, in game units (x1,000); the World Builder fit knows this term.",
                                                     render_block("TRY_INJECT:development", {"local_population_capacity": _fmt(k / 1000.0)})]) + "\n", encoding="utf-8", newline="\n")
    elif dev_path.is_file():
        dev_path.unlink()
