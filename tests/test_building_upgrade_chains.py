...
def _enabled_manifest_set(manifest: dict | None = None) -> set[str]:
...
def _is_generated_vanilla_copy(path: Path) -> bool:
    with path.open(encoding="utf-8-sig") as handle:
        return "Generated by ppc worldbuilder: vanilla file" in handle.readline()
...
def test_building_production_method_quantities_use_at_most_three_decimals() -> None:
    ...
    for path in paths:
        if path.suffix == ".txt" and _is_generated_vanilla_copy(path):
            continue   # vanilla copies with widened attribute tests keep vanilla's own quantities
