...
def test_building_levels_stripes_locations_above_supported_levels() -> None:
    ...
    assert _thresholds(block, "total_building_levels") == [
        10.0,
        25.0,
        50.0,
        75.0,
    ...
    assert "define:NMapColors|MAP_COLOR_TOP" not in block
    assert "rgb { 245 245 245 }" not in block
    assert "rgb { 170 235 255 }" not in block
    assert "rgb { 70 210 255 }" not in block
    assert "MAPMODE_PP_BUILDING_LEVELS_RANGE_150_200" in text
    assert "MAPMODE_PP_BUILDING_LEVELS_RANGE_250_300" in text
    assert "MAPMODE_PP_BUILDING_LEVELS_RANGE_400_500" in text
    assert "500 or more building levels" in text
    assert "secondary_map_color = {" in block
